...
Page({
    ...
    util.http('product/detail', {
      ...
      if (res.code == 200) {
        this.setData({
          ...
          shopthumb: res.data.pro_info.thumb,
          shoppeso: res.data.pro_info.peso,
  ...
  collect(e) {
    ...
    if (app.globalData.userInfo) {
      ...
      util.http('collect/change', {
        pid: this.data.good_id,
        type: collectType
      }, 'post', token).then(res => {})
    } else {
      main.goLogin(1)
  ...
  selectValue(attrValueList, index, key, value, unselectStatus) {
    ...
    let shopthumb = this.data.shopthumb;
    let shoppeso = this.data.shoppeso;
    if (contenArr.length == this.data.attrValueList.length) {
      shopStock = includeGroup[0].stock
      shopMoney = includeGroup[0].price
      shopthumb = includeGroup[0].thumb
      shoppeso = includeGroup[0].peso
    }
      ...
      if (comfirmCart == 1) {
        ...
        util.http('order/add', json, 'post', token).then(res => {
          if (res.code == 200) {
            ...
            wx.navigateTo({
              url: '../confirmationOrder/confirmationOrder?type=0&bargain=' + bargain
            })
          }
        })
      }
